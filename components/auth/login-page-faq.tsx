...
export const LoginPageFAQ = () => {
  ...
  return (
    <div className="flex mt-5 flex-col gap-10 justify-center items-center">
        ...
        <h2 className="text-center">Cant find an answer? Here{"'"}s our frequently asked questions</h2>
        <Accordion type="single" collapsible className="w-[80%] mb-20 md:w-[512px]">
            {faqs.map((faq, index) => (
                <AccordionItem className="px-5 my-2" value={`item-${index+1}`} key={index}>
                    <AccordionTrigger>
                        <h3>{faq.question}</h3>
                    </AccordionTrigger>
                    <AccordionContent>
