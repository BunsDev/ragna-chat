...
export const TrialChatBotComponent = ({
  chatId,
  dbMessages,
  isChatName,
}: TrialChatBotComponentProps) => {
  ...
  const preMessageState =
    (dbMessages && dbMessages.length === 1 && [systemMessage, ...dbMessages]) ||
    (!dbMessages && [systemMessage]) ||
    dbMessages;
  ...
  const feedbackLatest = (feedback: "GOOD" | "BAD") => {
    newLatestTrialFeedback(feedback).then((data) => {
      if (data.success) {
        toast({
  ...
  const refreshLatest = () => {
    const refreshMessages = messages.filter(
      (message, index) => index !== messages.length - 1
    );
  ...
  const fetchStream = async (newMessages: Message[]) => {
    ...
    try {
      ...
      if (!isChatName && messages.length > 3) {
        generateTitle(chatId, messages).then((data) => {
          if (data) {
